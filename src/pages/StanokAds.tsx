...
const AdCard: React.FC<{ a: any }> = ({ a }) => (
  <Card>
    <Link to={`/stanok-ads/${a.id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
...
export const StanokAds: React.FC = () => {
  ...
  if (filtered.length === 0) return <>{Header}<EmptyState icon="🏭" title="E'lonlar topilmadi" sub="Birinchi bo'lib e'lon joylang!" ctaLabel="+ E'lon joylash" onCta={() => nav('/stanok-ads/create')} /></>;

...
import { Avatar, Badge, Btn, Card, Chip, EmptyState } from '../ui';
import { api } from '../api';
...
export const WorkerProfile: React.FC = () => {
  ...
  if (error || !w) return <EmptyState icon="🔎" title="Ishchi topilmadi" sub={error} ctaLabel="Orqaga" onCta={() => nav('/workers')} />;
    ...
    <div style={{ maxWidth: 540, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 14 }}>
        ...
        <a href={w.telegram ? `https://t.me/${String(w.telegram).replace(/^@/, '')}` : w.phone ? `https://t.me/+${String(w.phone).replace(/\D/g, '')}` : undefined} target="_blank" rel="noreferrer" style={{ textDecoration: 'none', opacity: (w.telegram || w.phone) ? 1 : .5, pointerEvents: (w.telegram || w.phone) ? 'auto' : 'none' }}>
          <Btn variant="ghost" full style={{ color: 'var(--blue)', borderColor: 'var(--blue-100)' }}>✈ Telegram</Btn>
        </a>
      </div>
    </div>
  );

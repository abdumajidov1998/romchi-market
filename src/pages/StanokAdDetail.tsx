...
import { Badge, Btn, Card, EmptyState, TelegramIcon, tgHref } from '../ui';
...
export const StanokAdDetail: React.FC = () => {
  const { id } = useParams();
  ...
  if (error || !d) return <EmptyState icon="🔎" title="Topilmadi" sub={error} ctaLabel="Orqaga" onCta={() => nav('/stanok-ads')} />;
      ...
      {isOwner && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 10 }}>
            <Btn variant="ghost" full onClick={() => nav(`/stanok-ads/${d.id}/edit`)}>✏️ Tahrirlash</Btn>

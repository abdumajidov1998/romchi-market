...
export const Admin: React.FC = () => {
  ...
  React.useEffect(() => {
    if (!auth.token()) { nav('/welcome', { replace: true }); return; }
    api.me()
      .then(m => {
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '16px 14px', minHeight: '100vh' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0 14px' }}>
        <button onClick={() => nav('/')} style={{ width: 38, height: 38, borderRadius: 12, background: '#fff', border: '1px solid var(--line)', cursor: 'pointer' }}>

...
import { Badge, Btn, Card, Chip, EmptyState, formatUZS, TelegramIcon, tgHref } from '@/components/ui';
...
export default function JobDetail() {
  ...
  if (error || !j) return <EmptyState icon="🔎" title="Ish topilmadi" sub={error} ctaLabel="Orqaga" onCta={() => router.push('/jobs')} />;
      ...
      {isOwner ? (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 14 }}>
            <Btn full onClick={() => router.push(`/post?edit=${j.id}`)}>✏️ Tahrirlash</Btn>

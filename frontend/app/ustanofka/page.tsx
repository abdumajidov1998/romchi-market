...
import { Card, Btn, Chip, EmptyState, TelegramIcon, tgHref } from '@/components/ui';
import { usePersistedState } from '@/lib/persist';
import { useIsDesktop } from '@/components/Layout';
import { api } from '@/lib/api';
import { SpecIcon } from '@/components/SpecIcon';
import { INSTALL_BRIGADA_SPECS as SPECS } from '@/lib/constants';
...
export default function InstallBrigades() {
  const desktop = useIsDesktop();
  const router = useRouter();
  const [specs, setSpecs] = usePersistedState<string[]>('filters:installBrigades:specs', []);
  const [list, setList] = React.useState<any[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState('');
  const [nearbyOn, setNearbyOn] = usePersistedState('filters:installBrigades:nearbyOn', false);
  const [radiusKm, setRadiusKm] = usePersistedState<number>('filters:installBrigades:radiusKm', 15);
  ...
  const filtered = React.useMemo(() => {
    let result = list;
    if (specs.length > 0) {
      result = result.filter(b => (b.specs || []).some((s: string) => specs.includes(s)));
    }
      ...
      result = result
        .map(b => ({
          ...
          distanceKm: (typeof b.lat === 'number' && typeof b.lng === 'number')
            ? haversine(myCoords, { lat: b.lat, lng: b.lng }) : Infinity,
        }))
  ...
  const Header = (
    <>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0 14px' }}>
        ...
        <div style={{ textAlign: 'center', flex: 1 }}>
          <div style={{ fontSize: 12, color: 'var(--muted)', display: 'inline-flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
            <SectionIcon name="hardhat" size={14} /> Ustanofka xizmati
          </div>
          <div style={{ fontWeight: 800, fontSize: desktop ? 26 : 20 }}>Ustanofka brigadalar</div>
        </div>
        <div style={{ width: 38 }} />
      </div>
      ...
      <div style={{ fontSize: 13, color: 'var(--muted)', margin: '6px 2px 12px' }}>
        <b style={{ color: 'var(--ink)' }}>{filtered.length}</b> ta brigada
  ...
  if (filtered.length === 0) return <>{Header}<EmptyState icon="🏗️" title="Brigadalar topilmadi" sub={nearbyOn ? `${radiusKm} km radiusda topilmadi` : 'Birinchi bo\'lib ro\'yxatdan o\'ting!'} /></>;
  ...
  return (
    <div>
      ...
      <div style={{ display: desktop ? 'grid' : 'flex', flexDirection: 'column', gap: 12, gridTemplateColumns: desktop ? 'repeat(auto-fill, minmax(380px, 1fr))' : undefined }}>
        {filtered.map(b => <BrigadaCard key={b.id} b={b} />)}
      </div>
    </div>
  );
};

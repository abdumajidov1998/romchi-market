...
import { Btn, Input, Field, EmptyState } from '@/components/ui';
...
export default function CreateStanokAd() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/stanok-ads/${newId}`);
    } catch (e: any) { setError(e.message || 'Xatolik'); }
    finally { setSaving(false); }
  };

  if (loadingAd) return <EmptyState icon="⏳" title="Yuklanmoqda..." sub="" />;
  if (forbidden) return <EmptyState icon="🚫" title="Ruxsat yo'q" sub="Bu e'lonni faqat egasi tahrirlay oladi" ctaLabel="Orqaga" onCta={() => router.push('/stanok-ads')} />;
  if (loadError) return <EmptyState icon="⚠️" title="Xatolik" sub={loadError} ctaLabel="Orqaga" onCta={() => router.push('/stanok-ads')} />;
  ...
  return (
    <form onSubmit={submit} style={{ maxWidth: 540, margin: '0 auto', paddingBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0 14px' }}>
        <button type="button" onClick={() => router.push(isEdit && id ? `/stanok-ads/${id}` : '/stanok-ads')} style={{ width: 38, height: 38, borderRadius: 12, background: '#fff', border: '1px solid var(--line)', fontSize: 16, cursor: 'pointer' }}><img src="/images/back.png" alt="orqaga" style={{ width: 16, height: 16, display: 'block', margin: 'auto' }} /></button>

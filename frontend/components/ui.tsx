...
type BtnProps = { variant?: 'primary' | 'ghost' | 'soft' | 'dark'; full?: boolean; onClick?: () => void; children: React.ReactNode; style?: React.CSSProperties; type?: 'button' | 'submit'; [k: string]: any };
export const Btn: React.FC<BtnProps> = ({ variant = 'primary', full, onClick, children, style, type = 'button', ...rest }) => {
  ...
  return <button type={type} onClick={onClick} style={styles} {...rest}>{children}</button>;
};
...
export const WorkerCard: React.FC<{ worker: Worker; compact?: boolean }> = ({ worker, compact }) => (
  <Card>
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
      <Link href={`/workers/${worker.id}`} style={{ display: 'flex', gap: 12, flex: 1, minWidth: 0 }}>
...
export const JobCard: React.FC<{ job: Job }> = ({ job }) => (
  <Card>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
      <Link href={`/jobs/${job.id}`} style={{ flex: 1, display: 'flex', gap: 12 }}>
